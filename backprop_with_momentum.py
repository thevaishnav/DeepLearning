...
class Network:
    ...
    def back_prop(self, X, Y, batch_size, eta=0.05, alpha=0.9, epochs=5, epoch_complete_call=None):
        """
        ...
        for epch in range(epochs):
            ...
            for minix, miniy in mini_batches:
                ...
                self.update_weights(batchY, eta, alpha)

            if epoch_complete_call is not None:
                epoch_complete_call(epch)

    def update_weights(self, batchY, eta, alpha):
        """internal function"""
        ...
        for i in range(len(self.weights)):
            self.MWs[i] = alpha * self.MWs[i] + (1 - alpha) * deltaWs[i]
            self.MBs[i] = alpha * self.MBs[i] + (1 - alpha) * deltaBs[i]
            self.weights[i] -= eta * self.MWs[i]
            self.biases[i] -= eta * self.MBs[i]
