    ...
    def back_prop(self, X, Y, batch_size, eta=0.005, alpha=0.9, beta=0.999, epsilon=0.00005, epochs=5, epoch_complete_call=None):
        ...
        for epch in range(epochs):
            ...
            for minix, miniy in mini_batches:
                ...
                self.update_weights(batchY, eta, alpha, beta, epsilon)

            if epoch_complete_call is not None:
                epoch_complete_call(epch)

    def update_weights(self, batchY, eta, alpha, beta, epsilon):
        """internal function"""
        ...
        for i in range(len(self.weights)):
            ...
            self.biases[i] -= eta * self.SBs[i] / ((self.SBs[i] ** 0.5) + epsilon)
