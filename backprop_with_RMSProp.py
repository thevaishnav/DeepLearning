...
class Network:
    ...
    def init_parameters(self):
        ...
        self.GWs = [0, ] * len(self.weights)
        self.GBs = [0, ] * len(self.biases)
    ...
    def back_prop(self, X, Y, batch_size, eta=0.005, beta=0.9, epsilon=0.00005, epochs=5, epoch_complete_call=None):
        """
        ...
        for epch in range(epochs):
            ...
            for minix, miniy in mini_batches:
                ...
                self.update_weights(batchY, eta, beta, epsilon)

            if epoch_complete_call is not None:
                epoch_complete_call(epch)

    def update_weights(self, batchY, eta, beta, epsilon):
        """internal function"""
        ...
        for i in range(len(self.weights)):
            self.GWs[i] = beta * self.GWs[i] + (1 - beta) * (deltaWs[i] ** 2)
            self.GBs[i] = beta * self.GBs[i] + (1 - beta) * (deltaBs[i] ** 2)
            self.weights[i] -= eta * deltaWs[i] / ((self.GWs[i] ** 0.5) + epsilon)
            self.biases[i] -= eta * deltaBs[i] / ((self.GBs[i] ** 0.5) + epsilon)
